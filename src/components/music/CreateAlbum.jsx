    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
        ...
        <div className="bg-spotify-dark rounded-lg p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
            ...
            {availableTracks.length > 0 && (
              ...
              >
                {filteredTracks.length === formData.musics.length
                  ? "Deselect All"
                  : "Select All"}
              </button>
            )}
            ...
            <>
              ...
              <div className="mt-4 flex items-center justify-between text-sm">
                <span className="text-spotify-gray">
                  {formData.musics.length} of {availableTracks.length} track
                  {availableTracks.length !== 1 ? "s" : ""} selected
                </span>
                {formData.musics.length > 0 && (
                  <span className="text-spotify-green">✓ Ready to create</span>
                )}
              </div>
            </>
          )}
        ...
        <button
          type="submit"
          disabled={
            submitting || formData.musics.length === 0 || !formData.title
          }
        ...
        >
          {submitting
            ? "Creating Album..."
            : `Create Album with ${formData.musics.length} Track${formData.musics.length !== 1 ? "s" : ""}`}
        </button>
      </form>
    </div>
  );
